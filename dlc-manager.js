...
window.onload = () => {
  getAllDLC()
    .then(data => {
      ...
      const openDLCCountSpan = document.createElement('span');
      openDLCCountSpan.className = 'open-dlc-count';
      ...
      for (let i = allDLC.length - 1; i >= 0; i--) {
        ...
        newLi.innerText = `DLC ${i + 1} - blockNumber: ${allDLC[i].blockNumber}, timeStamp: ${allDLC[i].timeStamp},
         gasUsed: ${allDLC[i].gasUsed}, nonce: ${allDLC[i].nonce}, blockHash: ${allDLC[i].blockHash}`;
